...
RATES=($RATE10S $RATE1M $RATE1H)
WRATES=($WRATE10S $WRATE1M $WRATE1H)
...
if [ ${#MONDATA_FILES[*]} -eq 0 ]; then
	...
	echo "Metrics are sent to $SERVERIP:$SERVERPORT every $INTERVAL seconds."
	echo "File list with metric data:"
